...
class ResNetColorizeModel(nn.Module):

  def __init__(self):
    super(ResNetColorizeModel, self).__init__()
    ...
    if torch.cuda.is_available():  # and only if gpu is available
      resnet_gray_weights = torch.load(
        'pretrained/resnet_gray_weights.pth.tar')  # torch.load('pretrained/resnet_gray.tar')['state_dict']
    ...
    self.fusion_and_colorization_net = ResNetColorizationNet()
